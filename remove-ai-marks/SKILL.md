...
separate HTTP service (bundled here as `remove-ai-marks/service/`), so the agent
host needs no Python, venvs, or cleaning tools. Call the service with `curl`;
...
The service is started by the operator with the bundled launcher:

```bash
./remove-ai-marks/start-service.sh          # foreground, 127.0.0.1:8765
```

**Always check it first**, and stop with a clear message if it is unreachable —
never fall back to local cleaning:

```bash
curl -sf --noproxy 127.0.0.1 "$WM/health"
# {"ok": true, "version": "..."}
```

If the environment sets `HTTP_PROXY`/`http_proxy`, every `curl` below needs
`--noproxy 127.0.0.1` (or `export NO_PROXY=127.0.0.1,localhost`) or the request
is sent to the proxy and fails.
...
Or against the bundled copy: `python3 remove-ai-marks/service/scripts/audit_dir.py DIR --json`.
...
(`./remove-ai-marks/start-service.sh`). Do **not** attempt to clean locally —
this skill contains no cleaning code.

## Provenance

Vendored from [guillaumemeyer/watermarks-remover](https://github.com/guillaumemeyer/watermarks-remover)
(MIT), upstream commit `d5563d2`. See `NOTICE` and `INSTALL.md` in this
directory. Only the service paths and the proxy caveat were adapted; the
workflow, prompts, and caveats are upstream's.
